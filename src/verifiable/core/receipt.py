...
def generate_receipt_markdown_report(receipt: VerifiableReceipt) -> str:
    """Generate human-readable audit report for the receipt."""
    audit = receipt.independent_audit
    prov = receipt.provenance
    claim = receipt.claim

    return f"""# VERIFIABLE Receipt Report — {receipt.receipt_id}
...
## 2. Independent Audit (VERIFIABLE Independent Checker)
...
To reproduce this receipt independently using the VERIFIABLE CLI:

```bash
verifiable reproduce receipts/{receipt.receipt_id}
```
...
*Generated by VERIFIABLE Runtime v0.1.0*
"""
