...
1. **Birth Token (`verifier-birth-token-1`)**: Anchors identity creation epoch without leaking civil identity or activity telemetry.
2. **Aging Token (`verifier-aging-token-1`)**: Cryptographically proves continuous, unrevoked tenure across discrete epochs without leaking transaction history.
3. **Lifetime Token (`verifier-lifetime-token-1`)**: Grants ephemeral, soulbound capability leases for runner delegation with zero blast radius to genesis keys.
...
### 3.1 Birth Token (`verifier-birth-token-1`)

A Birth Token records the genesis commitment of an actor identity.

- **Wire Identifier:** `schema_version = "verifier-birth-token-1"`
...
### 3.2 Aging Token (`verifier-aging-token-1`)

An Aging Token proves continuous, unrevoked tenure across N discrete verification epochs.

- **Wire Identifier:** `schema_version = "verifier-aging-token-1"`
...
### 3.3 Lifetime Token (`verifier-lifetime-token-1`)

A Lifetime Token is an ephemeral, soulbound capability lease delegated to a runner worker or subagent.

- **Wire Identifier:** `schema_version = "verifier-lifetime-token-1"`
