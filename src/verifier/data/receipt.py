...
GRAPH receipt model and canonical serialization.
...
from verifier.data.policy import PolicyEvaluationResult
...
class VstdDataReceipt:
    ...
    def save_to_directory(self, out_dir: Path) -> Path:
        ...
        reproduce_py = (
            ...
            "from verifier.data.receipt import validate_data_receipt, reproduce_data_receipt\n\n"
...
def validate_data_receipt(receipt_path_or_dir: Path) -> int:
    """Validate a GRAPH-1 receipt without a target-specific adapter."""
    ...
    if missing:
        ...
        return 1
    if data.get("schema_version") != "verifier-data-1":
        print(
...
def generate_data_receipt_markdown(receipt: VstdDataReceipt) -> str:
    ...
    return (
        f"# VSTD Dataset Provenance Report — {receipt.receipt_id}\n\n"
...
*Generated by the GRAPH-1 reference runtime
(serialized receipt identifier `verifier-data-1`).*
"""
