...
| What can interoperability loading, analysis, and experimental planning establish? | Strict loading can establish that supplied VSTD-2 JSON has the supported structure and valid internal references; analysis identifies holes already represented by the resulting typed geometry; experimental planning identifies exact declared candidates in one bound component registry. | Supplied JSON, strict loader, typed geometry and digest, registry digest, exact schema, interaction-mode, relation, and mechanism coordinates, component descriptors, prerequisites, blockers, and the nonexecuting planner. | Successful loading does not establish the truth or completeness of the declared surface. A candidate is not checker execution, evidence, a native result, validation, closure, safety, authority to act, critical-domain readiness, or VSTD conformance. |
...
In the experimental interoperability path, **validation** names the future process of
attempting to discharge verification-surface holes with authorized component execution
...
| “GRAPH records this lineage graph.” | **Yes.** | The receipt binds the stored artifact nodes, transformation edges, roles, statuses, and declarations. Historical GRAPH-1 receipts retain the serialized receipt identifier `verifier-data-1`. | Valid receipt and structurally valid hypergraph. | The graph contains every real-world input or transformation. |
...
| “This registered component is an exact candidate for this modeled surface hole.” | **Yes, only after exact catalog matching in a registry-bound plan.** | The component's declared schema and interaction mode plus the hole's relation and mechanism coordinates, where present, matched exactly; domain tags did not participate. | Structurally valid typed geometry, geometry digest, exact hole, complete component descriptor, registry version and digest, planner implementation, candidate status, prerequisites, and blockers. | The component ran, was available at execution time, produced evidence, discharged the hole, was independently checked, established closure or safety, or is ready for a critical domain. |
...
> Using GRAPH-1 at commit `<commit>`, receipt `<receipt-id>` validated the stored
